...
class Solution {
    public int findRadius(int[] houses, int[] heaters) {
        ...
        int p = 0, q = 0;
        int radius = 0;
        while(p < houses.length) {
            while(q < heaters.length - 1 && Math.abs(heaters[q+1]-houses[p]) <= Math.abs(heaters[q] - houses[p])) ++q;
            radius = radius > Math.abs(heaters[q] - houses[p]) ? radius : Math.abs(heaters[q] - houses[p]);
            ++p;
        }
        return radius;
    }
